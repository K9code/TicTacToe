		...
		while (a == true) {
			...
			for (int l = 0; l< checkAi; l++) {
				...
				for (int b : freePosList) {
					...
					if (winningPos == 0); && (loopCounter < checkAi)) {
						tempCheckWinner(cpuPos);
						loopCounter += 1;
					}
					
					if (tempCheckWinner(cpuPos) || checkPos(playerPositions, cpuPositions, cpuPos)) {
						System.out.println("checkedPos");
//						cpuPos = rand.nextInt(9) + 1;
					
					}	
				}
				
				System.out.println("FreePosList = " + freePosList);
...
//				System.exit(0); --> funktioniert nicht
	...
	public static void checkWinnerPos() {
...
//	public static void checkPos(int cpuPos) {
//		if (playerPositions.contains(cpuPos) || tempCpuPositions.contains(cpuPos)) {
//			
//		}
//
//	}
	...
	public static boolean checkPos(ArrayList playerPositions, ArrayList cpuPositions, int cpuPos) {
		if (playerPositions.contains(cpuPos) || cpuPositions.contains(cpuPos)) return true; 
		return false;
	}
	...
	public static void tempCheckWinner(int cpuPos) {
		...
		ArrayList<Integer> tempPlayerPositions = new ArrayList<Integer>(); 
		ArrayList<Integer> tempCpuPositions = new ArrayList<Integer>();
		...
		tempCpuPositions = cpuPositions;
		tempCpuPositions.add(cpuPos);
		
		for (List l : winning) {
			if (tempCpuPositions.containsAll(l)) {
				winningPos = cpuPos;
				int lastInd = tempCpuPositions.lastIndexOf(cpuPos);
				tempCpuPositions.remove(lastInd);
